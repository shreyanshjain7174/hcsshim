...
func (h *Host) RemoveContainer(id string) {
	...
	criType, _ := c.spec.Annotations[annotations.KubernetesContainerType]
...
// setupMountsPath creates the sandboxMounts directory from a resolved root.
func setupMountsPath(sandboxRoot string) (err error) {
	mountPath := filepath.Join(sandboxRoot, "sandboxMounts")
...
// setupHugePageMountsPath creates the hugepages mounts directory from a resolved root.
func setupHugePageMountsPath(sandboxRoot string) error {
	mountPath := filepath.Join(sandboxRoot, "hugepages")
	if err := os.MkdirAll(mountPath, 0755); err != nil {
...
// setupLogDir creates the directory to house all redirected stdio logs from a resolved root.
func setupLogDir(sandboxRoot string) error {
	mountPath := filepath.Join(sandboxRoot, "logs")
...
func (h *Host) CreateContainer(ctx context.Context, id string, settings *prot.VMHostedContainerSettingsV2) (_ *Container, err error) {
		...
		case "sandbox":
			...
			if isVirtualPod {
				...
				if filepath.IsAbs(cleanID) || strings.Contains(cleanID, "..") {
					return nil, errors.Errorf("invalid virtual pod ID %q: contains path traversal", virtualPodID)
				}
				sandboxRoot = filepath.Join(filepath.Dir(settings.OCIBundlePath), "virtual-pods", cleanID)
			} else {
				sandboxRoot = settings.OCIBundlePath
			}
			h.RegisterSandboxRoot(id, sandboxRoot)
			...
			if err = setupMountsPath(sandboxRoot); err != nil {
				return nil, err
			}
			if err = setupTmpfsMountsPath(sandboxRoot); err != nil {
				return nil, err
			}
			if err = setupHugePageMountsPath(sandboxRoot); err != nil {
				return nil, err
			}
			if err = setupLogDir(sandboxRoot); err != nil {
				return nil, err
			}
	...
	if logDirMount := settings.OCISpecification.Annotations[annotations.LCOWTeeLogDirMount]; logDirMount != "" {
		settings.OCISpecification.Mounts = append(settings.OCISpecification.Mounts, specs.Mount{
			...
			Type:        "bind",
			Source:      filepath.Join(c.sandboxRoot, "logs"),
			Options:     []string{"bind"},
		})
	}
	...
	if logPath := settings.OCISpecification.Annotations[annotations.LCOWTeeLogPath]; logPath != "" {
		...
		c.logPath = filepath.Join(c.sandboxRoot, "logs", logPath)
		// verify the logpath is still under the correct directory
		logsDir := filepath.Join(c.sandboxRoot, "logs")
